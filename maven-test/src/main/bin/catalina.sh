...
exec "${JAVA_HOME}/bin/java" ${JAVA_OPTS} -server -Xms${HEAP_MEMORY} -Xmx${HEAP_MEMORY} -XX:+HeapDumpOnOutOfMemoryError -Xdebug -Xrunjdwp:transport=dt_socket,suspend=n,server=y,address=5454 -jar ${SERVER_JAR}